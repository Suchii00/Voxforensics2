...
interface AuthContextType {
  ...
  register: (email: string, password: string, name: string, referralCode?: string) => boolean;
  logout: () => void;
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const register = (email: string, password: string, name: string, referralCode?: string): boolean => {
    const users: User[] = JSON.parse(localStorage.getItem('voxforensics_users') || '[]');
    ...
    const newUser: User = {
      ...
      referralCode: referralCode || `VF-${Math.random().toString(36).substring(2, 8).toUpperCase()}`,
    ...
    localStorage.setItem('voxforensics_current_user', JSON.stringify(newUser));
    return true;
  };
...
export default function AuthSystem() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      ...
      const success = register(email, password, name, referralCode || undefined);
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="glass-panel p-8 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gradient mb-2">VoxForensics</h1>
        ...
        <div className="mt-6 pt-6 border-t border-[#1a2a4a]">
          ...
          <div className="space-y-2 text-xs">
            <div className="flex justify-between text-gray-400">
              <span>Admin:</span>
              <span className="font-mono">admin@voxforensics.com / admin123</span>
            </div>
            <div className="flex justify-between text-gray-400">
              <span>User:</span>
