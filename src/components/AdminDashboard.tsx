...
export default function AdminDashboard({ currentUser, onLogout, onBack }: { currentUser: User; onLogout: () => void; onBack: () => void }) {
  ...
  useEffect(() => {
    const allUsers: User[] = JSON.parse(localStorage.getItem('voxforensics_users') || '[]');
    setUsers(allUsers);
  }, []);
  ...
  const deleteUser = (userId: string) => {
    if (confirm('Are you sure you want to delete this user?')) {
      ...
      localStorage.setItem('voxforensics_users', JSON.stringify(updatedUsers));
    }
  };
    ...
    <div className="min-h-screen" style={{ background: '#030712' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="glass-card p-6">
                ...
                <p className="text-xs text-green-400 mt-2">↑ Active system</p>
              </div>
              <div className="glass-card p-6">
                <p className="text-xs text-gray-400 mb-1">Admin Users</p>
                <p className="text-3xl font-bold text-purple-400">{adminUsers}</p>
                <p className="text-xs text-gray-500 mt-2">With admin access</p>
              </div>
              <div className="glass-card p-6">
                <p className="text-xs text-gray-400 mb-1">Regular Users</p>
                <p className="text-3xl font-bold text-cyan-400">{regularUsers}</p>
                <p className="text-xs text-gray-500 mt-2">Standard access</p>
              </div>
              <div className="glass-card p-6">
                <p className="text-xs text-gray-400 mb-1">2FA Enabled</p>
                <p className="text-3xl font-bold text-green-400">{twoFactorEnabled}</p>
                <p className="text-xs text-gray-500 mt-2">{((twoFactorEnabled / totalUsers) * 100).toFixed(0)}% of users</p>
              </div>
            </div>
        ...
        {activeTab === 'analytics' && (
          <div className="space-y-6">
            <div className="glass-card p-6">
              <h2 className="text-lg font-bold text-white mb-4">📈 System Analytics</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-4 rounded-xl bg-white/5 border border-white/5">
                  <p className="text-sm text-gray-400 mb-2">User Growth</p>
                  <p className="text-2xl font-bold text-cyan-400">{totalUsers} Total</p>
                  <div className="mt-3 h-2 rounded-full bg-white/5 overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-cyan-500 to-purple-500" style={{ width: '100%' }}></div>
                  </div>
                </div>
                <div className="p-4 rounded-xl bg-white/5 border border-white/5">
                  <p className="text-sm text-gray-400 mb-2">Security Status</p>
                  <p className="text-2xl font-bold text-green-400">{((twoFactorEnabled / totalUsers) * 100).toFixed(0)}%</p>
                  <p className="text-xs text-gray-500">2FA adoption rate</p>
                </div>
                <div className="p-4 rounded-xl bg-white/5 border border-white/5">
                  <p className="text-sm text-gray-400 mb-2">Referral Program</p>
                  <p className="text-2xl font-bold text-yellow-400">
                    {users.filter(u => u.referredBy).length}
                  </p>
                  <p className="text-xs text-gray-500">Users joined via referral</p>
                </div>
                <div className="p-4 rounded-xl bg-white/5 border border-white/5">
        ...
        {activeTab === 'settings' && (
          <div className="glass-card p-6">
            <h2 className="text-lg font-bold text-white mb-4">⚙️ System Settings</h2>
                ...
                <button
                  onClick={() => {
                    if (confirm('This will delete ALL user data. Are you sure?')) {
                      localStorage.removeItem('voxforensics_users');
